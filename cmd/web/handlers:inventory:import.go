...
import (
	...
	"github.com/bit8bytes/gearberg/internal/inventory/imports"
...
type inventoryImportPreviewData struct {
	OrgID      string
	ImportID   string
	Rows       []imports.Row
	CountNew   int
...
// getInventoryImport serves the upload form when no ?id= param is present,
// or the staging preview when ?id= is set (after a successful upload).
func (app *application) getInventoryImport(w http.ResponseWriter, r *http.Request) *httperr.Error {
	orgID := r.PathValue("org_id")
	importID := r.URL.Query().Get("id")
	if importID == "" {
		data := app.html.TemplateData(r)
		data.Data = inventoryImportData{OrgID: orgID}
		return app.html.Render(w, r, http.StatusOK, pages.InventoryImport, data)
	}
	return app.renderImportPreview(w, r, orgID, importID)
}

func (app *application) postInventoryImport(w http.ResponseWriter, r *http.Request) *httperr.Error {
	ctx := r.Context()
	orgID := r.PathValue("org_id")

	reRender := func(msg string) *httperr.Error {
		data := app.html.TemplateData(r)
		data.Data = inventoryImportData{OrgID: orgID, Error: msg}
		return app.html.Render(w, r, http.StatusUnprocessableEntity, pages.InventoryImport, data)
	}
	...
	importID, err := app.services.inventoryImports.Stage(ctx, orgID, rawRows)
	if err != nil {
	...
	http.Redirect(w, r, "/orgs/"+url.PathEscape(orgID)+"/inventory/import?id="+url.QueryEscape(importID), http.StatusSeeOther)
	return nil
}
...
func (app *application) renderImportPreview(w http.ResponseWriter, r *http.Request, orgID, importID string) *httperr.Error {
	...
	rows, err := app.services.inventoryImports.ListStaged(ctx, importID)
	if err != nil {
	...
	data := app.html.TemplateData(r)
	data.Data = inventoryImportPreviewData{
		OrgID:      orgID,
		ImportID:   importID,
		Rows:       rows,
		CountNew:   cntNew,
	...
	return app.html.Render(w, r, http.StatusOK, pages.InventoryImportPreview, data)
}

func (app *application) postInventoryImportConfirm(w http.ResponseWriter, r *http.Request) *httperr.Error {
	ctx := r.Context()
	orgID := r.PathValue("org_id")
	...
	if err := app.services.inventoryImports.Commit(ctx, importID, orgID); err != nil {
...
// Column order: 0=Code, 1=Name, 2=Type, 3=Usage, 4=Category, 5=Manufacturer,
// 6=Total Stock, 7=Purchase Price, 8=Rental Price, 9=Notes, 10=Created At, 11=Updated At.
func readImportRows(cr *csv.Reader) ([]imports.RawRow, string) {
	...
	for {
		...
		rows = append(rows, imports.RawRow{
			...
			UsageTypeLabel:   strings.TrimSpace(record[3]),
			CategoryName:     strings.TrimSpace(record[4]),
			ManufacturerName: strings.TrimSpace(record[5]),
			TotalStock:       strings.TrimSpace(record[6]),
			PurchasePrice:    strings.TrimSpace(record[7]),
			RentalPrice:      strings.TrimSpace(record[8]),
			Notes:            strings.TrimSpace(record[9]),
		})
	}
	if len(rows) == 0 {
