...
import (
	...
	genloc "github.com/bit8bytes/gearberg/internal/database/queries/gen/locations"
)
...
// Delete removes the location. Inventory items referencing it will have their location_id set to NULL.
...
func toModel(row genloc.Location) Location {
	return Location{
		ID:        row.ID,
		OrgID:     row.OrgID,
		Name:      row.Name,
